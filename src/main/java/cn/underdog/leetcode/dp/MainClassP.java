...
//        climbStairs(9);
        MainClassP mainClassP = new MainClassP();
//        mainClassP.maximumDifference(new int[]{9, 4, 4, 2});
//        mainClassP.climbStairsReview(3);
        mainClassP.minCostClimbingStairsReview(new int[]{10, 15, 20});
    }
   ...
   /* public static int maxSubArray(int[] nums) {
    ...
    public int maximumDifference(int[] nums) {
        ...
        for (int i = 1; i < length; i++) {
            int current = nums[i];
            res = Math.max(res, Math.max(current - nums[i - 1], current - min));
            if (min > current)
                min = current;
        }
        return res == 0 ? -1 : res;
     ...
     */
    /**
     * 子序列问题，dp定义为以i为结尾的出现的连续子序列，如果
     *
     * @param nums
     * @return
     */
    /*public int maxSubArrayReview(int[] nums) {
        int length = nums.length;
        // dp为以i结尾当前出现的最大的连续的和
     ...
     * @return
     */
    public int minCostClimbingStairsReview(int[] cost) {
